...
def i4_for_level(
    ...
    if level == Level.NATIONAL:
        return i4_national(timespan, environment)
...
def i4_national(timespan: IndicatorTimeSpan, environment: Environment) -> pd.DataFrame:
    """Calculate i4 at the national level."""
    with Session(get_api_db_engine(environment)) as session:
        result = session.execute(text("SELECT COUNT(*) FROM Station"))
        count = result.one()[0]
    indicators = {
        "target": "00",
        "value": [count],
...
def calculate(  # noqa: PLR0913
    ...
    subflows_results = [
        i4_for_level(level, timespan, environment, chunk_size=chunk_size)
        for level in levels
    ]
