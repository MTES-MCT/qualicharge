...
E3: the list of points of charge of the RTE-T network.
"""
...
SELECT
    id_station_itinerance as id_station,
    max(puissance_nominale) as p_max,
    sum(puissance_nominale) as p_cum,
...
@flow(
    ...
    flow_run_name="e3-{timespan.period.value}-{level:02d}-{timespan.start:%y-%m-%d}",
)
def e3_for_level(
    level: Level,
    timespan: IndicatorTimeSpan,
...
@flow(
    task_runner=ThreadPoolTaskRunner(max_workers=settings.THREAD_POOL_MAX_WORKERS),
    flow_run_name="e3-{timespan.period.value}-00-{timespan.start:%y-%m-%d}",
)
def e3_national(
    timespan: IndicatorTimeSpan,
    timespan_query: IndicatorTimeSpan,
    ...
    """Calculate e3 at the national level."""
    ...
    extras_list = list(result["id_station_itinerance"])
    indicators = {
        "target": "00",
...
@flow(
    task_runner=ThreadPoolTaskRunner(max_workers=settings.THREAD_POOL_MAX_WORKERS),
    flow_run_name="meta-e3-{period.value}",
)
def calculate(  # noqa: PLR0913
    ...
    subflows_results = [
        e3_for_level(level, timespan, environment, chunk_size=chunk_size)
        for level in levels
    ]
    indicators = pd.concat(subflows_results, ignore_index=True)
    description = f"e3 report at {timespan.start} (period: {timespan.period.value})"
