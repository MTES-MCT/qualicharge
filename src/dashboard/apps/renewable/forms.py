...
class RenewableReadingForm(ModelForm):
    ...
    def _clean_collected_at(
        ...
        if collected_date > end_date:
            self.field_errors[collected_at_key] = mark_safe(  # noqa: S308
                _(
                    f"The date cannot be in the future. <br />"
                    f"Collected date should be between {min_date.strftime("%d/%m/%Y")} "
            ...
            self.field_errors[collected_at_key] = mark_safe(  # noqa: S308
                _(
                    f"The date cannot be earlier than {min_date.strftime("%d/%m/%Y")}."
                    f"<br />"
                    f"Collected date should be between {min_date.strftime("%d/%m/%Y")} "
                    f"and {end_date.strftime("%d/%m/%Y")}."
                )
