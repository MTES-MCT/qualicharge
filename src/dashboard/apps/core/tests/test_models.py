...
def test_get_unsubmitted_quarterly_renewables(monkeypatch):
    ...
    # Create a previous renewable to test `last_meter_reading` and `last_collected_at`
    ...
    # test the `last_meter_reading` and `last_collected_at` fields
    first_dp = renewable_dps.get(id=dps[1].id)
    assert first_dp.last_meter_reading == previous_meter_reading
    assert first_dp.last_collected_at.strftime("%Y-%m-%d") == previous_date
