...
def sync_entity_from_siret(siret: str, user: Optional[DashboardUser] = None) -> Entity:
    ...
    company_info_client: CompanyInformationClient = CompanyInformationClient()
    company_address_client: CompanyAddressClient = CompanyAddressClient()

    company_info: CompanyInformationAdapter = company_info_client.get(siren)
    company_address: CompanyAddressAdapter = company_address_client.get(
